...
def test_start_progress_sets_started_at(point_progress_repo, point_progress, existing_datetime):
    updated_count = point_progress_repo.start_progress(point_progress.id, existing_datetime)
...
def test_start_progress_uses_current_time_by_default(point_progress_repo, point_progress):
...
def test_start_progress_does_not_affect_other_point_progresses(
    ...
    other_from_db = point_progress_repo.get_by_id(other_point_progress.id)
    assert other_from_db.started_at is None
...
def test_finish_progress_sets_finished_at_and_is_finished(
        point_progress_repo, point_progress, existing_datetime,
):
    updated_count = point_progress_repo.finish_progress(point_progress.id, existing_datetime)
...
def test_finish_progress_uses_current_time_by_default(point_progress_repo, point_progress):
    updated_count = point_progress_repo.finish_progress(point_progress.id)
...
def test_finish_progress_does_not_affect_other_point_progresses(
    ...
    other_from_db = point_progress_repo.get_by_id(other_point_progress.id)
    assert other_from_db.finished_at is None
    assert other_from_db.is_finished is False
...
def test_get_non_finished_by_session_returns_only_non_finished(point_progress_repo, user_session):
...
def test_get_non_finished_by_session_returns_empty_when_no_progresses(point_progress_repo, user_session):
    result = point_progress_repo.get_non_finished_by_session(user_session.id)
...
def test_has_non_finished_returns_true_when_non_finished_exists(point_progress_repo, user_session):
    PointProgress.create(user_session=user_session, is_finished=False)

    assert point_progress_repo.has_non_finished(user_session.id) is True


def test_has_non_finished_returns_false_when_all_finished(point_progress_repo, user_session):
    PointProgress.create(user_session=user_session, is_finished=True)

    assert point_progress_repo.has_non_finished(user_session.id) is False


def test_has_non_finished_returns_false_when_no_progresses(point_progress_repo, user_session):
    assert point_progress_repo.has_non_finished(user_session.id) is False
...
def test_get_current_by_session_returns_none_if_session_without_current_point(
        point_progress_repo, user_session,
):
    assert point_progress_repo.get_current_by_session(user_session) is None


def test_get_current_by_session_returns_none_when_no_progress_for_session(
        point_progress_repo, other_user_session,
):
    assert point_progress_repo.get_current_by_session(other_user_session) is None


# ── increment_clues_used (CAS-инкремент с лимитом) ────────────────────────────
...
def test_increment_clues_used_does_not_affect_other_point_progresses(
    ...
    other_from_db = point_progress_repo.get_by_id(other_point_progress.id)
    assert other_from_db.clues_used == 0


def test_increment_clues_used_returns_none_if_point_progress_not_found(point_progress_repo):
