...
# ── update_role / update_state (параметризовано через UPDATE_METHODS) ─────────
...
def test_increment_recovery_attempts_returns_zero_if_user_not_found(user_repo):
...
def test_reset_recovery_attempts_sets_to_zero(user_repo, user):
    ...
    updated_count = user_repo.reset_recovery_attempts(user.id)
...
def test_reset_recovery_attempts_returns_zero_if_user_not_found(user_repo):
    updated_count = user_repo.reset_recovery_attempts(999)

    assert updated_count == 0


# ── set_error / clear_error / reset_ui_msg_id (параметризовано) ───────────────
...
def test_update_current_session_returns_zero_if_user_not_found(user_repo, user_session):
    updated_count = user_repo.update_current_session(999, user_session.id)

    assert updated_count == 0


# ── update_if_field_equals (CAS) ──────────────────────────────────────────────

def test_update_if_field_equals_updates_when_value_matches(user_repo, user):
    result = user_repo.update_if_field_equals(
        user.id, "state", UserState.idle, UserState.registration,
    )
...
def test_update_if_field_equals_returns_false_when_user_not_found(user_repo):
    result = user_repo.update_if_field_equals(
        999, "state", UserState.idle, UserState.registration,
    )
...
def test_update_if_field_equals_does_not_affect_other_users(user_repo, user, other_user):
    ...
    user_repo.update_if_field_equals(
        user.id, "state", UserState.idle, UserState.registration,
    )
...
def test_update_if_field_equals_protects_from_double_update(user_repo, user):
    ...
    result1 = user_repo.update_if_field_equals(
        user.id, "state", UserState.idle, UserState.registration,
    )
    # Второй "процесс" пытается сделать то же — но значение уже изменилось
    result2 = user_repo.update_if_field_equals(
        user.id, "state", UserState.idle, UserState.registration,
    )
